...
The application is designed to work with pins set up as Buttons, LEDs, and GPIOs by the Device Configurator tool (part of ModusToolbox&#8482;, launched from the IDE QuickPanel or by command line from the application project folder with "make config"). Buttons are a list of GPIO inputs connected to buttons, LEDs are a list of GPIO outputs connected to LEDs, and GPIOs that are not connected to Buttons or LEDs are in a separate list and may be configured as input or output. The lists are created by the Device Configurator. The application monitors Buttons and GPIO inputs for changes and prints a message to the puart console on each change. The application toggles LEDs and GPIO outputs periodically. The period is changed after each input event, switching between LED\_BLINK\_FREQ\_A\_IN\_SECONDS and LED\_BLINK\_FREQ\_B\_IN\_SECONDS.

This application can also be used to experiment with Device Configurator creating different pin configurations. For example, when the application starts it prints a report on the pins found in each of the Button, LED, and GPIO lists. Input pins can be connected with a fly-wire to ground or VIO to view the event messages. Output pins can be monitored with a volt meter or fly-wired to LEDs to observe voltage toggles. When changing the default configuration of Buttons, LEDs, and GIOS take care to keep indexes consistent in each list (1, 2, ...) and not to exceed WICED\_PLATFORM\_*\_MAX. Also, when changing the default pin configurations be sure to review the kit schematic to ensure the pin has no additional connections that may interfere with the input or output operations.
